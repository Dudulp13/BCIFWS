...
if (isset($_POST["save_announce"])) {
    ...
    $announceDateTime = $conn->real_escape_string($_POST["time_date"]);
    ...
    if ($conn->query($insert_query)) {
        ...
        // Redirect with success message, including the announcement ID
        header("Location: announce.php?status=success&message=New announcement has been successfully added! <br>ID: $announcement_id");
    } else {
        // Redirect with error message
        header("Location: announce.php?status=error&message=Failed to add announcement: " . urlencode($conn->error));
    }
    exit;
}
?>